...
/**
 * Multilevel taskbar plugin for fbpanel
 * Based on the taskbar plugin.
 *
 * Modified : 
...
typedef struct _mltaskbar{
    ...
    GdkPixbuf *gen_pixbuf, *gen_pixbuf_high, *gen_pixbuf_low, *gen_pixbuf_other;
...
static gchar *mltaskbar_rc = "style 'mltaskbar-style'\n"
"{\n"
...
"}\n"
"widget '*.mltaskbar.*' style 'mltaskbar-style'";
...
static void tk_display(mltaskbar_priv *tb, task *tk);
static void tb_propertynotify(mltaskbar_priv *tb, XEvent *ev);
static GdkFilterReturn tb_event_filter( XEvent *, GdkEvent *, mltaskbar_priv *);
static void mltaskbar_destructor(plugin_instance *p);
...
static void
tk_get_level(task *tk)
{
	unsigned int tmp;
	Status ret;

	ret = XSecurityQueryTrustLevel(GDK_DISPLAY(), tk->win, &tmp);
	if (ret == (Status)0) 
...
static int
task_visible(mltaskbar_priv *tb, task *tk)
{
    ENTER;
...
static task *
find_task (mltaskbar_priv * tb, Window win)
{
    ENTER;
...
static void
del_task (mltaskbar_priv * tb, task *tk, int hdel)
{
    ENTER;
...
get_generic_icon(mltaskbar_priv *tb, task *tk)
{
    ENTER;
    switch (tk->level) {
        ...
        case CLIP_LEVEL_HIGH:
            g_object_ref(tb->gen_pixbuf_high);
            RET(tb->gen_pixbuf_high);
            break;
        case CLIP_LEVEL_LOW:
            g_object_ref(tb->gen_pixbuf_low);
            RET(tb->gen_pixbuf_low);
            break;
	default:
            g_object_ref(tb->gen_pixbuf_other);
            RET(tb->gen_pixbuf_other);
            break;
    }    
}

static void
tk_update_icon (mltaskbar_priv *tb, task *tk, Atom a)
{
    GdkPixbuf *pixbuf;
    ...
    if (tk->level != CLIP_LEVEL_CLIP) {
	ERR("level != CLIP_LEVEL_CLIP\n");
        tk->pixbuf = get_generic_icon(tb, tk);
        goto out;
    }

    /* level == CLIP_LEVEL_CLIP */
    ...
    if (!tk->pixbuf) {
        tk->pixbuf = get_generic_icon(tb, tk); // always exists
        DBGE("generic_icon=1");
    }
...
static void
tk_update(gpointer key, task *tk, mltaskbar_priv *tb)
{
    ENTER;
...
static void
tk_display(mltaskbar_priv *tb, task *tk)
{    
    ENTER;
...
static void
tk_build_gui(mltaskbar_priv *tb, task *tk)
{
    GtkWidget *w1;
...
static void
tb_net_client_list(GtkWidget *widget, mltaskbar_priv *tb)
{
    int i;
    task *tk;
...
static void
tb_net_current_desktop(GtkWidget *widget, mltaskbar_priv *tb)
{
    ENTER;
...
static void
tb_net_number_of_desktops(GtkWidget *widget, mltaskbar_priv *tb)
{
    ENTER;
...
static void
tb_net_active_window(GtkWidget *widget, mltaskbar_priv *tb)
{
    Window *f;
    task *ntk, *ctk;
...
static void
tb_propertynotify(mltaskbar_priv *tb, XEvent *ev)
{
    Atom at;
    Window win;
...
tb_event_filter( XEvent *xev, GdkEvent *event, mltaskbar_priv *tb)
{
...
static void
menu_close_window(GtkWidget *widget, mltaskbar_priv *tb)
{
    ENTER;    
    DBG("win %x\n", tb->menutask->win);
    XSync (GDK_DISPLAY(), 0);
...
static void
menu_raise_window(GtkWidget *widget, mltaskbar_priv *tb)
{
    ENTER;    
    DBG("win %x\n", tb->menutask->win);
...
static void
menu_iconify_window(GtkWidget *widget, mltaskbar_priv *tb)
{
    ENTER;    
    DBG("win %x\n", tb->menutask->win);
...
static GtkWidget *
mltaskbar_make_menu(mltaskbar_priv *tb)
{
    GtkWidget *mi, *menu;
...
static void
mltaskbar_build_gui(plugin_instance *p)
{
    mltaskbar_priv *tb = (mltaskbar_priv *) p;
    ...
    tb->gen_pixbuf_high =  gdk_pixbuf_new_from_xpm_data((const char **)icon_high_xpm);
    tb->gen_pixbuf_low =  gdk_pixbuf_new_from_xpm_data((const char **)icon_low_xpm);
    tb->gen_pixbuf_other =  gdk_pixbuf_new_from_xpm_data((const char **)icon_other_xpm);
    ...
    tb->menu = mltaskbar_make_menu(tb);
...
int
mltaskbar_constructor(plugin_instance *p)
{
    mltaskbar_priv *tb;
    GtkRequisition req;
    xconf *xc = p->xc;
    ...
    ENTER;
    tb = (mltaskbar_priv *) p;
    gtk_rc_parse_string(mltaskbar_rc);
...
static void
mltaskbar_destructor(plugin_instance *p)
{
    mltaskbar_priv *tb = (mltaskbar_priv *) p;
...
static plugin_class class = {
    ...
    .name        = "mltaskbar",
    .version     = "1.1",
    .description = "Multi-level version of the taskbar plugin",
    .priv_size   = sizeof(mltaskbar_priv),
    
    .constructor = mltaskbar_constructor,
    .destructor  = mltaskbar_destructor,
};
